#!/bin/bash
# Usage: ./generate-all.sh "apache/superset:3.1.arm" "tech4dev/superset:0.41" "output_folder"
...
BASE_IMAGE=$1
OUTPUT_IMAGE=$2
OUTPUT_FOLDER="first-script-output"

# Create the output folder if it doesn't exist
mkdir -p $OUTPUT_FOLDER

# Generate the Dockerfile
cat <<EOF > ${OUTPUT_FOLDER}/Dockerfile
...
USER superset
EOF
echo "${OUTPUT_IMAGE}" > image_info.txt
